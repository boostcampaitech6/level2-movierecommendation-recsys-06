...
def filter_triplets(tp, min_uc=5, min_sc=0):
    ...
    unique_uid = usercount.user # 아무리봐도 여기 이상한데?
    np.random.seed(98765)
    idx_perm = np.random.permutation(unique_uid.size)
    unique_uid = unique_uid[idx_perm]

    return tp, usercount, itemcount, unique_uid
...
def split_data(args, unique_uid, raw_data):
...
def numerize_write(args, profile2id, show2id, raw_data, train_plays, vad_plays_tr, vad_plays_te, test_plays_tr, test_plays_te):
    ...
    with open('json_id/show2id.json', 'w') as json_file :
        json.dump(show2id, json_file)
    
    with open('json_id/profile2id.json', 'w') as json_file :
        json.dump(show2id, json_file)

    with open('json_id/id2show.json', 'w') as json_file :
